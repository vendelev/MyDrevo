...
5. Данные успешно сохраняются в SQLite.
6. Тесты покрывают основные сценарии.

## Дополнительные тесты для миграций
- **Тест миграции gen_person**: Проверка добавления полей `gender`, `birth_place`, `death_place`, `created_at`, `updated_at`
- **Тест миграции gen_relation**: Проверка добавления поля `metadata`
- **Тест существующего поля INFO**: Проверка, что поле `INFO` в таблице `gen_person_info_lang` доступно для использования
